...
boolean selectedbeing = false, selectedbless = false, selectedCLEAR = false;
float clearsize = 100;
    ...
    if(displaybeingmenu && !(mouseX > 250 && mouseX < 850 && mouseY < 750 && mouseY > 600)){
      if(selectedbeing){
        ...
        //if(!(mouseX > 250 && mouseX < 850 && mouseY < 750 && mouseY > 600)){
          ...
          totalbeings += 1;
        //}
      }else if(selectedCLEAR){
        //if(!(mouseX > 250 && mouseX < 850 && mouseY < 750 && mouseY > 600)){
          for(int bg = beings.size() - 1; bg >= 0 ; bg--){
            being thisbeing = beings.get(bg);
            if(dist(mouseX, mouseY, thisbeing.x, thisbeing.y) < clearsize/2){
                beings.remove(bg);
                totalbeings -= 1;
            }
          }
          for(int bl = blesses.size() - 1; bl >= 0; bl--){
            blessedbeing thisbless = blesses.get(bl);
            if(dist(mouseX, mouseY, thisbless.x, thisbless.y) < clearsize/2){
                blesses.remove(bl);
                totalbeings -= 1;
            }
          }
...
void mouseWheel(MouseEvent event) {
  ...
  clearsize = max(min(clearsize, 500),30);
}
